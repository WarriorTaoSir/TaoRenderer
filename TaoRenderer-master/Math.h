...
/*
	计算观察变换矩阵——详见RTR4 4.1.6
	核心思想是，先将相机平移到坐标原点，然后再对基底进行变换，
	使得向量r指向（1,0,0），向量u指向（0,1,0），向量v指向（0,0,-1）这样相机看向-z
	注 ： 右手坐标系
